...
## Conclusion
In this assignment, we discussed a possible attack against the DAC-MACS attribute-based encryption scheme, a recent advance in cryptography with practical applications to secure document storage and other modern problemsm mainly concerning enterprise users. We showed in our proof-of-concept example that by knowing x_2, Alice was able to fully decrypt the message intended for only Bob. From the equations it is clear that by keeping this value hidden from Alice, the ABE scheme works as intended. As a sanity check, we ran honest decryptions as well and showed that they work as they should.
